...
ok($dpCU->can("sort_by_column"), 'sort_by_column');# 9
ok($dpCU->can("seen_one_column"), 'seen_one_column');# 10
ok($dpCU->can("select_rows"), 'select_rows');# 11
ok($dpCU->can("print_to_screen"), 'print_to_screen');# 12
ok($dpCU->can("print_to_file"), 'print_to_file');# 13
ok($dpCU->can("print_with_delimiter"), 'print_with_delimiter');# 14
ok($dpCU->can("full_report"), 'full_report');# 15
ok($dpCU->can("writeformat"), 'writeformat');# 16
ok($dpCU->can("writeformat_plus_header"), 'writeformat_plus_header');# 17
ok($dpCU->can("writedelimited"), 'writedelimited');# 18
ok($dpCU->can("writedelimited_plus_header"), 'writedelimited_plus_header');# 19
ok($dpCU->can("writeHTML"), 'writeHTML');# 20

# 4.02:  Get information about the Data::Presenter::Combo::Union object itself.

ok( ($dpCU->print_data_count), 'print_data_count');# 21
ok( ($dpCU->get_data_count == 19), 'get_data_count');# 22
%seen = map { $_ => 1 } @{$dpCU->get_keys};

ok($seen{210297}, 'key recognized');    # 23
ok($seen{359962}, 'key recognized');    # 24
ok($seen{392877}, 'key recognized');    # 25
ok($seen{399723}, 'key recognized');    # 26
ok($seen{399901}, 'key recognized');    # 27
ok($seen{456600}, 'key recognized');    # 28
ok($seen{456787}, 'key recognized');    # 29
ok($seen{456788}, 'key recognized');    # 30
ok($seen{456789}, 'key recognized');    # 31
ok($seen{456790}, 'key recognized');    # 32
ok($seen{456791}, 'key recognized');    # 33
ok($seen{456792}, 'key recognized');    # 34
ok($seen{456892}, 'key recognized');    # 35
ok($seen{458732}, 'key recognized');    # 36
ok($seen{498703}, 'key recognized');    # 37
ok($seen{698389}, 'key recognized');    # 38
ok($seen{786792}, 'key recognized');    # 39
ok($seen{803092}, 'key recognized');    # 40
ok($seen{906786}, 'key recognized');    # 41
ok(! $seen{987654}, 'key correctly not recognized');# 42
ok(! $seen{123456}, 'key correctly not recognized');# 43
ok(! $seen{333333}, 'key correctly not recognized');# 44
ok(! $seen{135799}, 'key correctly not recognized');# 45

# 4.03:  Call simple output methods on Data::Presenter::Combo::Union object:
$return = $dpCU->print_to_screen();
ok( ($return == 1), 'print_to_screen'); # 46

$outputfile = "$resultsdir/census30.txt";
$return = $dpCU->print_to_file($outputfile);
ok( ($return == 1), 'print_to_file');   # 47

$outputfile = "$resultsdir/census30_delimited.txt";
$delimiter = '|||';
$return = $dpCU->print_with_delimiter($outputfile, $delimiter);
ok( ($return == 1), 'print_with_delimiter');# 48

$outputfile = "$resultsdir/report30.txt";
$return = $dpCU->full_report($outputfile);
ok( ($return == 1), 'full_report');     # 49
...
ok( (1 == sdtest(\@columns_selected, $sorted_data)), 'valid sorted data hash');# 50
...
ok( ($return == 0), 'seen_one_column: 0 args'); # 51

{
    local $SIG{__WARN__} = \&_capture;
    $return = $dpCU->seen_one_column('unit', 'ward');
}
ok( ($return == 0), 'seen_one_column: 2 args'); # 52

%seen = %{$dpCU->seen_one_column('unit')};
ok( ($seen{'SAMSON'} == 3), 'seen_one_column:  1 arg');# 53
ok( ($seen{'LAVER'}  == 6), 'seen_one_column:  1 arg');# 54
ok( ($seen{'TRE'}    == 2), 'seen_one_column:  1 arg');# 55
...
ok( ($return == 1), 'writedelimited_plus_header');# 59

$outputfile = "$resultsdir/report30.html";
$title = 'Agency Census Report';
$return = $dpCU->writeHTML(
	$sorted_data, \@columns_selected, $outputfile, $title);
ok( ($return == 1), 'writeHTML');       # 60
...
%seen = ();
ok( ($dpCU->print_data_count), 'print_data_count');# 61
ok( ($dpCU->get_data_count == 12), 'get_data_count');# 62
%seen = map { $_ => 1 } @{$dpCU->get_keys};

ok($seen{359962}, 'key recognized');    # 63
ok($seen{392877}, 'key recognized');    # 64
ok($seen{456600}, 'key recognized');    # 65
ok($seen{456787}, 'key recognized');    # 66
ok($seen{456788}, 'key recognized');    # 67
ok($seen{456789}, 'key recognized');    # 68
ok($seen{456790}, 'key recognized');    # 69
ok($seen{456792}, 'key recognized');    # 70
ok($seen{498703}, 'key recognized');    # 71
ok($seen{698389}, 'key recognized');    # 72
ok($seen{786792}, 'key recognized');    # 73
ok($seen{906786}, 'key recognized');    # 74
ok(! $seen{210297}, 'key correctly not recognized');# 75
ok(! $seen{399723}, 'key correctly not recognized');# 76
ok(! $seen{399901}, 'key correctly not recognized');# 77

$return = $dpCU->print_to_screen();
ok( ($return == 1), 'print_to_screen'); # 78

$outputfile = "$resultsdir/combo_u_ward_200_plus.txt";
$return = $dpCU->print_to_file($outputfile);
ok( ($return == 1), 'print_to_file');   # 79
...
$sorted_data = $dpCU->sort_by_column(\@columns_selected);
ok( (1 == sdtest(\@columns_selected, $sorted_data)), 'valid sorted data hash');# 80
